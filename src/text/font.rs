        ...
        unsafe {
            ...
            match path {
                Some(path) => {
                    ...
                    if !freetype::FT_New_Face(font.library, c_str.as_ptr(), 0, &mut font.face)
                        .succeeded()
                    {
                ...
                None => {
                    if !freetype::FT_New_Memory_Face(
                        font.library,
                        &memory[0],
                        memory.len() as freetype::FT_Long,
                        0,
                        &mut font.face,
                    ).succeeded()
                    {
            ...
            for curr in 0usize..128 {
                ...
                let advance = Vector2::new(
                    ((*ft_glyph).advance.x >> 6) as f32,
                    ((*ft_glyph).advance.y >> 6) as f32,
                );
                let dimensions = Vector2::new(
                    (*ft_glyph).bitmap.width as f32,
                    (*ft_glyph).bitmap.rows as f32,
                );
                let offset = Vector2::new(
                    (*ft_glyph).bitmap_left as f32,
                    (*ft_glyph).bitmap_top as f32,
                );
            ...
            font.atlas_dimensions.x =
                (cmp::max(font.atlas_dimensions.x, row_width as usize)).next_power_of_two();
