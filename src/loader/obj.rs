...
/// Parses an obj file.
pub fn parse_file(path: &Path, mtl_base_dir: &Path, basename: &str) -> IoResult<Vec<(~str, Mesh, Option<MtlMaterial>)>> {
...
pub fn parse(string: &str, mtl_base_dir: &Path, basename: &str) -> Vec<(~str, Mesh, Option<MtlMaterial>)> {
